...
void person_dtor(Person *p) {
    ...
    free(p->name);
    p->name = NULL;
    // FIXED chyba! neni uvolnovana pamet pro jmeno
}
...
void array_dtor(PersonArray *a) {
    ...
    // FIXED chyba! Je potreba zlikvidovat postupne vsechny zaznamy.
}
...
unsigned int array_find_min(PersonArray *a, int idx) {
    ...
    for (unsigned int i = idx + 1; i < a->len; i++) {
...
void array_sort(PersonArray *a) {
    for (unsigned int i = 0; i < a->len - 1; i++) {
        int min = array_find_min(a, i);
        person_swap(&a->data[i], &a->data[min]);
    }
}

bool is_sorted(PersonArray *a) {
    for (unsigned i = 1; i < a->len; i++) {
        if (person_cmp(&a->data[i - 1], &a->data[i]) > 0) {
            return false;
        }
    }
    return true;
...
void array_shuffle(PersonArray *a) {
    // randomly swap two elements
    unsigned i = rand() % a->len;
    unsigned j = rand() % a->len;
    person_swap(&a->data[i], &a->data[j]);
}

void array_sort_bogo(PersonArray *a) {
    int tries = 0;
    srand(time(NULL));
    while (!is_sorted(a)) {
        array_shuffle(a);
    ...
    printf("Pole serazeno po %d pokusech\n", tries);
}
...
int main() {
    ...
    bool isSorted = is_sorted(&a);
    printf("Pole je serazene: %s\n", isSorted ? "ano" : "ne");
